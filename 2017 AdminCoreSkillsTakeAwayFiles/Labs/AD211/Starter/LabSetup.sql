...
SELECT TOP(100000) so.CustomerOrderID,od.Date AS OrderDate, dd.Date AS DeliveryDate, c.CinemaGroup, c.TradingName AS CinemaName, p.Description AS ProductName,
       so.ShippedOuters, so.ShippedUnits, so.PricePerOuter, so.PricePerUnit, so.UnitsPerOuter
INTO Sales.ShippedOrders
FROM PopkornKrazeDW.Fact.ShippedOrder AS so
INNER JOIN PopkornKrazeDW.Dimension.Product AS p
ON so.ProductKey = p.ProductKey
INNER JOIN PopkornKrazeDW.Dimension.Date AS od 
ON so.OrderDateKey = od.DateKey
INNER JOIN PopkornKrazeDW.Dimension.Date AS dd 
ON so.DeliveryDateKey = od.DateKey
INNER JOIN PopkornKrazeDW.Dimension.Cinema AS c
ON so.CinemaKey = c.CinemaKey
ORDER BY so.CustomerOrderID DESC;
GO
